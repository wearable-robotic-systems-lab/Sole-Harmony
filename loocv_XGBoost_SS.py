"""
Sole-HARmony — LOOCV (Sitting vs Standing)
...
df_all = load_files_for_prefix(train_subjects, "_features_rev2.csv")
...
df_all = df_all[df_all['label_Cam'].isin([0,1])].reset_index(drop=True)
...
def eval_sbj(subject_dfs, test_idx):
    ...
    filename_booster = f"XGB_SS_{subjects[test_idx]}.json"
    ...
    filename_meta = f"XGB_SS_{subjects[test_idx]}_meta.json"
