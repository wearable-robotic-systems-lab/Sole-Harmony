"""
Sole-HARmony — LOOCV (Stairs Up vs Down)
...
df_all = df_all[df_all['label_Cam'].isin([3,4])].reset_index(drop=True)
df_all['label_Cam'] = df_all['label_Cam'].map({3: 0, 4:1})
...
def eval_sbj(subject_dfs, test_idx):
    ...
    'colsample_bytree' : Real(0.2,1,prior='log-uniform'), #0.7
    ...
    filename_booster = f"XGB_sAD_{subjects[test_idx]}.json"
    ...
    filename_meta = f"XGB_sAD_{subjects[test_idx]}_meta.json"
...
for i in range(num_subjects):
    res = eval_sbj(subject_dfs, i)   # <— no writer passed
    results.append(res)
