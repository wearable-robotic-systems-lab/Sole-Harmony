"""
Sole-HARmony — Static vs Dynamic (XGBoost classifier)

Author: Laura De Marzi (Stevens Institute of Technology, WRSLab)

Trains a binary XGBoost classifier (static vs dynamic activities) using extracted
...
os.makedirs("XGBoost_StaticDynamic", exist_ok=True)
...
df_train_all['label_Cam'] = df_train_all['label_Cam'].map({0: 0, 1: 0, 2: 1, 3: 1, 4: 1})
df_test['label_Cam'] = df_test['label_Cam'].map({0: 0, 1: 0, 2: 1, 3: 1, 4: 1})
...
booster.save_model("XGBoost_StaticDynamic/XGB_SD.json")
...
with open("XGBoost_StaticDynamic/XGB_SD_meta.json", "w") as f:
