"""
Sole-HARmony — Stairs Up vs Down (XGBoost classifier)

Trains a binary XGBoost classifier to distinguish stair ascent vs descent
...
os.makedirs("ext_XGBoost_sAD", exist_ok=True)
...
df_train_all = df_train_all[df_train_all['label_Cam'].isin([3,4])].reset_index(drop=True)
df_train_all['label_Cam'] = df_train_all['label_Cam'].map({3: 0, 4: 1})

df_train_all = df_train_all.loc[:, ~df_train_all.columns.str.startswith(('label_AP'))]

df_test = df_test[df_test['label_Cam'].isin([3,4])].reset_index(drop=True)
df_test['label_Cam'] = df_test['label_Cam'].map({3: 0, 4: 1})
...
parameters = {
    ...
    'reg_lambda': Real(0.05,3,prior='log-uniform'),
    'scale_pos_weight': Real(0.5,1.5,prior='log-uniform'), 
...
booster.save_model("ext_XGBoost_sAD/XGB_sAD.json")

meta = {
    "best_iteration": int(best_iter)
}
with open("ext_XGBoost_sAD/XGB_sAD_meta.json", "w") as f:
