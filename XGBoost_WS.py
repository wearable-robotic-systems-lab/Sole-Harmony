"""
Sole-HARmony — Walking vs Stairs (XGBoost classifier)

Author: Laura De Marzi (Stevens Institute of Technology, WRSLab)

Trains a binary XGBoost classifier to distinguish walking from stair ambulation
using extracted insole features. Includes Bayesian hyperparameter optimization,
early stopping based on macro-F1, and evaluation on unseen subjects.

Input:  FeatureTables/*_features_rev2.csv
...
os.makedirs("XGBoost_WalkingStairs", exist_ok=True)
...
df_train_all = df_train_all[df_train_all['label_Cam'].isin([2,3,4])].reset_index(drop=True)
df_train_all['label_Cam'] = df_train_all['label_Cam'].map({2: 0, 3: 1, 4:1})

df_train_all = df_train_all.loc[:, ~df_train_all.columns.str.startswith(('label_AP'))]

df_test = df_test[df_test['label_Cam'].isin([2,3,4])].reset_index(drop=True)
df_test['label_Cam'] = df_test['label_Cam'].map({2: 0, 3: 1, 4:1})

# Include only features from IMU or FSR
# cols_to_keep = df_train_all.columns[~df_train_all.columns.str.contains("Acc|Gyr|Cor")]

# df_train_all = df_train_all[list(cols_to_keep)] # + ["label_Cam","subject","t_window_ms"]
# df_test = df_test[list(cols_to_keep)] # + ["label_Cam","subject","t_window_ms"]
############
...
parameters = {
    ...
    'gamma': Integer(3,5,prior='uniform'),
    'reg_alpha': Real(0.05,3,prior='log-uniform'), 
    'reg_lambda': Real(0.1,3,prior='log-uniform'), 
    'scale_pos_weight':  Real(12,15,prior='log-uniform'),
...
booster.save_model("XGBoost_WalkingStairs/XGB_WS.json")
...
with open("XGBoost_WalkingStairs/XGB_WS_meta.json", "w") as f:
