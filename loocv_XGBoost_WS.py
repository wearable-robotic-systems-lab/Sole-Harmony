"""
Sole-HARmony — LOOCV (Walking vs Stairs)
...
df_all = df_all[df_all['label_Cam'].isin([2,3,4])].reset_index(drop=True)
df_all['label_Cam'] = df_all['label_Cam'].map({2: 0, 3: 1, 4:1})
...
def eval_sbj(subject_dfs, test_idx):
    ...
    'min_child_weight': Integer(1, 50, prior='uniform')  #10
    }
    ...
    filename_booster = f"XGB_WS_{subjects[test_idx]}.json"
    ...
    filename_meta = f"XGB_WS_{subjects[test_idx]}_meta.json"
